/*******************************************************************************
* File Name: SOUND_SAMPLE_CLOCK.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: SOUND_SAMPLE_CLOCK_Start
********************************************************************************
*
* Summary:
...
void SOUND_SAMPLE_CLOCK_Start(void) 
{
    ...
    SOUND_SAMPLE_CLOCK_CLKEN |= SOUND_SAMPLE_CLOCK_CLKEN_MASK;
	SOUND_SAMPLE_CLOCK_CLKSTBY |= SOUND_SAMPLE_CLOCK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: SOUND_SAMPLE_CLOCK_Stop
********************************************************************************
*
* Summary:
...
void SOUND_SAMPLE_CLOCK_Stop(void) 
{
    ...
    SOUND_SAMPLE_CLOCK_CLKEN &= (uint8)(~SOUND_SAMPLE_CLOCK_CLKEN_MASK);
	SOUND_SAMPLE_CLOCK_CLKSTBY &= (uint8)(~SOUND_SAMPLE_CLOCK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: SOUND_SAMPLE_CLOCK_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void SOUND_SAMPLE_CLOCK_StopBlock(void) 
{
    if ((SOUND_SAMPLE_CLOCK_CLKEN & SOUND_SAMPLE_CLOCK_CLKEN_MASK) != 0u)
    {
...
#if defined(SOUND_SAMPLE_CLOCK__CFG3)
        CLK_DIST_AMASK = SOUND_SAMPLE_CLOCK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = SOUND_SAMPLE_CLOCK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* SOUND_SAMPLE_CLOCK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(SOUND_SAMPLE_CLOCK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(SOUND_SAMPLE_CLOCK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: SOUND_SAMPLE_CLOCK_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void SOUND_SAMPLE_CLOCK_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        SOUND_SAMPLE_CLOCK_CLKSTBY &= (uint8)(~SOUND_SAMPLE_CLOCK_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: SOUND_SAMPLE_CLOCK_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void SOUND_SAMPLE_CLOCK_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = SOUND_SAMPLE_CLOCK_GetSourceRegister();
    uint16 oldDivider = SOUND_SAMPLE_CLOCK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = SOUND_SAMPLE_CLOCK_CLKEN & SOUND_SAMPLE_CLOCK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(SOUND_SAMPLE_CLOCK_DIV_PTR, clkDivider);
                SOUND_SAMPLE_CLOCK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                SOUND_SAMPLE_CLOCK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(SOUND_SAMPLE_CLOCK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((SOUND_SAMPLE_CLOCK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(SOUND_SAMPLE_CLOCK_DIV_PTR, clkDivider);
				SOUND_SAMPLE_CLOCK_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: SOUND_SAMPLE_CLOCK_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 SOUND_SAMPLE_CLOCK_GetDividerRegister(void) 
{
    return CY_GET_REG16(SOUND_SAMPLE_CLOCK_DIV_PTR);
}


/*******************************************************************************
* Function Name: SOUND_SAMPLE_CLOCK_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void SOUND_SAMPLE_CLOCK_SetModeRegister(uint8 modeBitMask) 
{
    SOUND_SAMPLE_CLOCK_MOD_SRC |= modeBitMask & (uint8)SOUND_SAMPLE_CLOCK_MODE_MASK;
}


/*******************************************************************************
* Function Name: SOUND_SAMPLE_CLOCK_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void SOUND_SAMPLE_CLOCK_ClearModeRegister(uint8 modeBitMask) 
{
    SOUND_SAMPLE_CLOCK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(SOUND_SAMPLE_CLOCK_MODE_MASK));
}


/*******************************************************************************
* Function Name: SOUND_SAMPLE_CLOCK_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 SOUND_SAMPLE_CLOCK_GetModeRegister(void) 
{
    return SOUND_SAMPLE_CLOCK_MOD_SRC & (uint8)(SOUND_SAMPLE_CLOCK_MODE_MASK);
}


/*******************************************************************************
* Function Name: SOUND_SAMPLE_CLOCK_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void SOUND_SAMPLE_CLOCK_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = SOUND_SAMPLE_CLOCK_GetDividerRegister();
    uint8 oldSrc = SOUND_SAMPLE_CLOCK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        SOUND_SAMPLE_CLOCK_MOD_SRC |= CYCLK_SSS;
        SOUND_SAMPLE_CLOCK_MOD_SRC =
            (SOUND_SAMPLE_CLOCK_MOD_SRC & (uint8)(~SOUND_SAMPLE_CLOCK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: SOUND_SAMPLE_CLOCK_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 SOUND_SAMPLE_CLOCK_GetSourceRegister(void) 
{
    return SOUND_SAMPLE_CLOCK_MOD_SRC & SOUND_SAMPLE_CLOCK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: SOUND_SAMPLE_CLOCK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void SOUND_SAMPLE_CLOCK_SetPhaseRegister(uint8 clkPhase) 
{
    SOUND_SAMPLE_CLOCK_PHASE = clkPhase & SOUND_SAMPLE_CLOCK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: SOUND_SAMPLE_CLOCK_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 SOUND_SAMPLE_CLOCK_GetPhaseRegister(void) 
{
    return SOUND_SAMPLE_CLOCK_PHASE & SOUND_SAMPLE_CLOCK_PHASE_MASK;
}
