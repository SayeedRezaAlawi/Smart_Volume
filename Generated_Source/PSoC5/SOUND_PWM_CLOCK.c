...
/*******************************************************************************
* Function Name: SOUND_PWM_CLOCK_Start
********************************************************************************
*
* Summary:
...
void SOUND_PWM_CLOCK_Start(void) 
{
    ...
    SOUND_PWM_CLOCK_CLKEN |= SOUND_PWM_CLOCK_CLKEN_MASK;
	SOUND_PWM_CLOCK_CLKSTBY |= SOUND_PWM_CLOCK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: SOUND_PWM_CLOCK_Stop
********************************************************************************
*
* Summary:
...
void SOUND_PWM_CLOCK_Stop(void) 
{
    ...
    SOUND_PWM_CLOCK_CLKEN &= (uint8)(~SOUND_PWM_CLOCK_CLKEN_MASK);
	SOUND_PWM_CLOCK_CLKSTBY &= (uint8)(~SOUND_PWM_CLOCK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: SOUND_PWM_CLOCK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((SOUND_PWM_CLOCK_CLKEN & SOUND_PWM_CLOCK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = SOUND_PWM_CLOCK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = SOUND_PWM_CLOCK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* SOUND_PWM_CLOCK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(SOUND_PWM_CLOCK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(SOUND_PWM_CLOCK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: SOUND_PWM_CLOCK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SOUND_PWM_CLOCK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = SOUND_PWM_CLOCK_GetSourceRegister();
    uint16 oldDivider = SOUND_PWM_CLOCK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = SOUND_PWM_CLOCK_CLKEN & SOUND_PWM_CLOCK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(SOUND_PWM_CLOCK_DIV_PTR, clkDivider);
                SOUND_PWM_CLOCK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                SOUND_PWM_CLOCK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(SOUND_PWM_CLOCK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((SOUND_PWM_CLOCK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: SOUND_PWM_CLOCK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(SOUND_PWM_CLOCK_DIV_PTR);
}


/*******************************************************************************
* Function Name: SOUND_PWM_CLOCK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    SOUND_PWM_CLOCK_MOD_SRC |= modeBitMask & (uint8)SOUND_PWM_CLOCK_MODE_MASK;
}


/*******************************************************************************
* Function Name: SOUND_PWM_CLOCK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    SOUND_PWM_CLOCK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(SOUND_PWM_CLOCK_MODE_MASK));
}


/*******************************************************************************
* Function Name: SOUND_PWM_CLOCK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return SOUND_PWM_CLOCK_MOD_SRC & (uint8)(SOUND_PWM_CLOCK_MODE_MASK);
}


/*******************************************************************************
* Function Name: SOUND_PWM_CLOCK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = SOUND_PWM_CLOCK_GetDividerRegister();
    uint8 oldSrc = SOUND_PWM_CLOCK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        SOUND_PWM_CLOCK_MOD_SRC |= CYCLK_SSS;
        SOUND_PWM_CLOCK_MOD_SRC =
            (SOUND_PWM_CLOCK_MOD_SRC & (uint8)(~SOUND_PWM_CLOCK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: SOUND_PWM_CLOCK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return SOUND_PWM_CLOCK_MOD_SRC & SOUND_PWM_CLOCK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: SOUND_PWM_CLOCK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    SOUND_PWM_CLOCK_PHASE = clkPhase & SOUND_PWM_CLOCK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: SOUND_PWM_CLOCK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return SOUND_PWM_CLOCK_PHASE & SOUND_PWM_CLOCK_PHASE_MASK;
}
